...
class QA_CNN(object):
	def __init__(self,max_input_left,max_input_right,batch_size,vocab_size,embeddings,embedding_size,
		...
		with tf.name_scope('attention'):	
			self.U = tf.Variable(tf.truncated_normal(shape = [self.batch_size,self.num_filter,self.num_filter],stddev = 0.01,name = 'U'))
	...
	def attentive_pooling(self,input_left,input_right):
		...
		# Apply nonlinearity
		Q = tf.reshape(tf.nn.relu(tf.nn.bias_add(conv, self.conv_b),name = 'relu'),[self.batch_size,-1,self.num_filter])
		...
		# Apply nonlinearity
		A = tf.reshape(tf.nn.tanh(tf.nn.bias_add(conv, self.conv_b),name = 'relu'),[self.batch_size,-1,self.num_filter])
